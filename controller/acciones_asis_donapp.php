...
/**
 * Notifica al cliente por correo cuando cambia el estado de su donación/solicitud.
 */
function notificarCliente($conn, $tipo, $idRegistro, $nuevoEstado, $observacion) {
    ...
    try {
        ...
        <div style='font-family: sans-serif; padding: 24px; border: 1px solid #e0e0e0; border-radius: 10px; max-width: 520px; margin: auto;'>
            <h2 style='color: #1565c0; margin-bottom: 4px;'>DONAPP</h2>
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!in_array($nuevoEstado, $estadosValidos)) {
        redir('../view/asis_dashboard.php#donapp', 'Estado no válido.');
    }

    if ($nuevoEstado === 'rechazada' && trim($observacion) === '') {
        redir('../view/asis_dashboard.php#donapp', 'La observación es obligatoria al rechazar.');
    }

    // Registrar al asistente actual como gestor
    ...
    if ($tipo === 'donacion') {
        ...
        redir('../view/asis_dashboard.php#donapp', 'Donación actualizada correctamente.');
        ...
        redir('../view/asis_dashboard.php#donapp', 'Solicitud actualizada correctamente.');

    } else {
        redir('../view/asis_dashboard.php#donapp', 'Tipo de acción no reconocido.');
    }
}

redir('../view/asis_dashboard.php#donapp');